...
const icon = themeToggle.querySelector('i');// Función para cambiar el tema DARK/LIGTH de tailw
...
setTheme(savedTheme); themeToggle.addEventListener('click', () => {
...
//swiper.slideTo(swiper.slides.length - 1, 0); //* esto lo hice para stilar la ultima pantalla y cada vez que refresque queria verla directamente//!Logica de CAPTURA DE DATOS en pantalla usando swiper-jsconst userResponses = {};// Referencias a los botones de navegación
const nextButton = document.querySelector('.swiper-button-next');
const prevButton = document.querySelector('.swiper-button-prev');
const calculateButton = document.getElementById('calculate-button');
const backButton = document.getElementById('back-button');// Función universal para validar y manejar el estado del formulario actual
function handleForm() {
    const currentSlide = swiper.slides[swiper.realIndex];
    const currentForm = currentSlide.querySelector('form');    // Maneja la visibilidad de los botones de navegación y del botón de cálculo
    ...
    const requiredInputs = currentForm.querySelectorAll('[required]'); const checkValidity = () => {
...
swiper.on('slideChange', () => {
    const previousSlideIndex = swiper.previousIndex; switch (previousSlideIndex) {
        case 1:
...
const ARBOLES_POR_TONELADA_CO2 = 5; // Un árbol promedio compensa 200 kg de CO2 al año//! LOGICA DE CALCULO// Función para manejar el botón de cálculo
const handleCalculateButton = () => {
    ...
    const datosAlimentacion = userResponses.alimentacion;    // 2. Calcular la huella de carbono de cada sección
    const huellaTransporte = datosTransporte
        ...
        : 0; const huellaHogar = datosEnergia
            ...
            : 0; const huellaAlimentacion = datosAlimentacion
                ? (datosAlimentacion.carne * FACTORES_CO2_ANUAL.CARNE_KG * 12) +
                (datosAlimentacion.vegetales * FACTORES_CO2_ANUAL.VEGETALES_KG * 12)
                : 0;    // 3. Sumar para obtener la huella total
    const huellaTotal = huellaTransporte + huellaHogar + huellaAlimentacion;
    console.log(huellaTotal)    // 4. Calcular los árboles necesarios
    const arboles = Math.ceil((huellaTotal / 1000) * ARBOLES_POR_TONELADA_CO2);    // 5. Mostrar los resultados en la interfaz
    ...
    document.getElementById('arboles-compensacion').textContent = `${arboles} árboles`;    // Opcional: Desplazarse al slide de resultados
...
const handleBackButton = () => {
    ...
    const formAlimentos = document.getElementById('alimentacion'); if (formTransporte) formTransporte.reset();
    if (formEnergia) formEnergia.reset();
    if (formAlimentos) formAlimentos.reset(); swiper.slideTo(1); //Hace volver a pantalla 2
}; if (backButton) {
